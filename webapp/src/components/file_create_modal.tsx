  ...
  return (
    <Modal show={visible} onHide={handleClose} backdrop={'static'}>
      <Modal.Header closeButton={true}>
        <h4 className="modal-title">{`Create new ${templateType} from template.`}</h4>
      </Modal.Header>
      <Modal.Body>
        <div className="d-flex">
          <div className="form-control d-flex collabora-filename-container">
            ...
            />
            {fileName && (
              <div style={{ right: 5 }} className={clsx('input-clear', { visible: fileName.length > 0 })} onMouseDown={onClear} onTouchEnd={onClear}>
                <OverlayTrigger delayShow={400} placement={'bottom'} overlay={<Tooltip id={'InputClearTooltip'}>{'Clear'}</Tooltip>}>
      ...
      <Modal.Footer>
        <button type="button" className="btn btn-link cancel" onClick={handleClose}>
          {'Cancel'}
        </button>
        <button
          type="submit"
