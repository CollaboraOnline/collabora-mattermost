...
export const WopiFilePreview: FC<Props> = (props: Props) => {
  ...
  const handleWopiFile = useCallback(
  ...
  if (loading) {
    return (
      <span id="loadingSpinner" className={'wopi-loading-spinner'}>
        <i className="fa fa-spinner fa-fw fa-pulse spinner" title={'Loading Icon'} />
      </span>
    );
  }
  ...
  return (
    <div className="wopi-iframe-container">
      ...
      <iframe id="collabora-iframe" name="collabora-iframe" className="wopi-iframe" />
    </div>
  );
};
